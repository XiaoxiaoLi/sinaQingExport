...
@author: Ahan, modified by Xiaoxiao Li on 2015-06-06 for Sina Qing
...
#正则表达式定义
#匹配博文标题链接
pattern2=u"""<a href="(.*?)" class="title" title="(.*?)">"""
prog2=re.compile(pattern2)
#匹配下一页链接
pattern3=u"""<a class="button right" href="(.*?)"></a>"""
prog3=re.compile(pattern3)
#匹配正文部分
pattern4=u"""(<ul class="posts">.*?)<!--feed描述 start-->"""
...
def save_to_file(url,filename,blog_address):
    ...
    print "read data from url done"
    text=_filter(text)
    print "filter main text done"
    #将图片保存到本地
    ...
    for pic in result:
        ...
        if os.path.exists(folder)==False:
            os.makedirs(folder)
        try:
            url_file = urlopen(pic[0])
            print "pic url openned"
            pic_file = codecs.open(folder+pic_name,'wb')
            print "pic file openned"
            while True:
            ...
            #替换正文中的图片地址
            text=text.replace(pic[0],unicode("src=\"" + filename + "/" + pic_name + "\"" + pic[0]),1)
            i=i+1
...
def _filter(t):
    ...
    if result is not None:
        print unicode(result.group())
        return u'<html><head></head><body>' + unicode(result.group()) + u'</dody></html>'
    else:
        #raise Exception('噢，提取正文出错了……')
        raise Exception('Error in extracting main content from blog... Content is None.')
...
if __name__ == '__main__':
    ...
    text=read_date_from_url('http://qing.blog.sina.com.cn//'+blog_address)
    time.sleep(0.4)
    ...
    while True:
        ...
        for blog in result: 
            ...
            if blog_no < begin:
                ...
                try:
                    save_to_file(blog_url,unicode(title),blog_address)
                except:
                    #print '噢，保存第',blog_no,'篇博文',blog[0],'的时候出现问题了，跳过...'
                    print 'Error in saving the ',blog_no,'th blog post',title,'Skipping this one'
                    blog_no += 1
                    print "Unexpected error:", sys.exc_info()[0],sys.exc_info()[1]
                else:
                    #print '成功保存了第', blog_no, '篇博文:', blog[0]
                    print 'successfully saved the ', blog_no, 'th blog post:', title
                    blog_no += 1
                    saved += 1
                    time.sleep(0.6)
        #判断是否有下一页
